  ...
  return (
    <div className="mx-auto max-w-5xl p-4 sm:p-8 md:p-12">
      <header className="mb-10 flex flex-col justify-between gap-4 md:flex-row md:items-end">
        <div>
          <h1 className="text-3xl font-black uppercase tracking-tighter sm:text-4xl">Store Settings</h1>
      ...
      <form action={updateStoreSettings} className="grid grid-cols-1 gap-5 border border-gray-800 bg-neutral-900/50 p-4 sm:p-6 md:grid-cols-2 md:p-8">
        {[
          ...
          <label key={name} className="space-y-2">
            ...
            <input name={name} required={name !== "instagramUrl" && name !== "tiktokUrl" && name !== "twitterUrl"} defaultValue={value} className="min-h-12 w-full border border-gray-800 bg-black px-4 py-3 text-base text-white sm:text-sm" />
          </label>
        ))}
        ...
        <label className="space-y-2">
          ...
          <input name="freeShippingThresholdCents" type="number" inputMode="numeric" min="0" defaultValue={content.freeShippingThresholdCents} className="min-h-12 w-full border border-gray-800 bg-black px-4 py-3 text-base text-white sm:text-sm" />
        </label>
        <label className="space-y-2">
          <span className="text-[10px] font-black uppercase tracking-[0.24em] text-gray-500">Flat shipping, cents</span>
          <input name="flatShippingCents" type="number" inputMode="numeric" min="0" defaultValue={content.flatShippingCents} className="min-h-12 w-full border border-gray-800 bg-black px-4 py-3 text-base text-white sm:text-sm" />
        </label>
