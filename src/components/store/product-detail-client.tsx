...
export default function ProductDetailClient({ product }: { product: any }) {
  ...
  return (
    <div className="flex flex-col justify-center">
      ...
      <p className="text-3xl text-gray-400 font-bold mb-8 italic">{formatCurrency(product.priceCents, product.currency)}</p>
      ...
      <div className="space-y-8 max-w-md">
        ...
        <div className="pt-8 border-t border-gray-900">
          <div className="flex justify-between items-center mb-6">
            ...
            <button className="text-[9px] font-black uppercase tracking-widest text-white/50 hover:text-white transition-colors underline underline-offset-4">Size Guide</button>
          </div>
          ...
          <div className="grid grid-cols-4 gap-3">
            {product.variants.map((v: any) => (
              <button 
                ...
                className={`py-4 border text-[10px] font-black uppercase tracking-widest rounded-xl transition-all duration-300
                  ${v.stock > 0 
                    ? selectedSize === v.size
        ...
        <div className="pt-8 flex gap-4">
          <button 
            ...
            disabled={isAdding}
            className={`flex-1 py-5 rounded-2xl font-black uppercase tracking-[0.2em] text-xs transition-all duration-500 flex items-center justify-center gap-3
              ${isAdding 
